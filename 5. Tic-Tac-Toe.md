...
- `transfer::receive(&mut game.id, mark)`로 `Receiving` Mark를 게임이 대리 수령합니다.
...
1. `mark_mut(game, row, col)` → (row, col) 좌표의 보드 칸을 가리키는 가변 참조(`&mut u8`)를 반환합니ㅏㄷ.
2. `*mark_mut(...)` → 참조 해제해서 실제 칸 값에 접근
3. `= sentinel` → 그 위치에 sentinel 값을 대입 (예: `MARK_X` 또는 `MARK_O`)
...
- 승리(`TROPHY_WIN`): 승자에게 트로피 전송, GameEnd 이벤트 기록
- 무승부(`TROPHY_DRAW`): 양쪽 모두에게 트로피 전송, GameEnd 이벤트 기록
- 진행(`TROPHY_NONE`): 새 TurnCap 만들어 다음 플레이어에게 전송
...
```bash
sui client ptb \
  --sender @0xd3116c130f7c3d76866251610d0aaf1be2a36860b21415515083c43fd728bce9 \
  --make-move-vec "<u8>" "[]" \
  --assign admin_vec \
  --move-call 0x3756d0d5c50c3233772405d527bcc64f4a10a4f003fdead5761c11a568b2b2d3::owned::new \
    @0xbc9dc570e8ad7ee3f1321634735e3e647586b75386e85771a35d711619ec454a \
    @0x3ad23d3702b4ed6044d2d8f6c5aa016157585e019039cb2b4bba431c68f4d2d6 \
    admin_vec \
  --assign game \
  --transfer-objects "[game]" @0xd3116c130f7c3d76866251610d0aaf1be2a36860b21415515083c43fd728bce9 \
  --gas-budget 100000000
```

### 5-3 계정 변경

sui client switch --address 0xbc9dc570e8ad7ee3f1321634735e3e647586b75386e85771a35d711619ec454a

sui client call \
 --package 0x3756d0d5c50c3233772405d527bcc64f4a10a4f003fdead5761c11a568b2b2d3 \
 --module owned \
 --function send_mark \
 --args 0x5bf453ad2db67e34c1eef4889f7716ecd57b1401377959454bd3c235e449688b 0 0

sui client switch --address 0xd3116c130f7c3d76866251610d0aaf1be2a36860b21415515083c43fd728bce9

sui client call \
 --package 0x3756d0d5c50c3233772405d527bcc64f4a10a4f003fdead5761c11a568b2b2d3 \
 --module owned \
 --function place_mark \
 --args 0x80a5b04d23c04ebfe5de9f03e77b0e2222cdb33763f0098e363eea0a11fa4d8f 0xbcdfa647c4e8cbf9ee5f3c89632f61b79dbaafde432f17257d40aefd814adfd4
