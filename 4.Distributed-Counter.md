...
> `Request rejected 429` 에러는 `Too Many Requests.` 에러로 발생시 배포 명령을 다시 실행해야 합니다. 배포가 계속 안될 경우 `--gas-budget`의 값을 높여주세요.
...
> `Request rejected 429` 에러가 발생하면 다시 시도하거나 gas-budget을 올려서 시도해봅니다.
